...
class MPMCSampler:
    def __init__(self, x, log_w, rho, mu, sigma, z_est, mu_est):
        self.particles = x          # samples
        self.log_weights = log_w    # log weights
        self.mix_weights = rho      # weight of each mixture component
        self.means = mu             # mean of each mixture component
        self.covariances = sigma    # covariance matrix of each mixture component
        self.evidence = z_est       # unbiased estimate of the normalization constant
        self.target_mean = mu_est   # self-normalized estimate of the target mean (MMSE)
...
class APISSampler:
    def __init__(self, x, log_w, mu, z_est, mu_est):
        self.particles = x          # samples
        self.log_weights = log_w    # log weights
        self.means = mu             # locations of the population of proposals
        self.evidence = z_est       # unbiased estimate of the normalization constant
        self.target_mean = mu_est   # self-normalized estimate of the target mean (MMSE)
...
class MCMCSampler:
    def __init__(self, x):
        self.samples = x            # samples from the target distribution (MCMC chain)
...
def metropolis_hastings(log_target, initial_state, var_x=1, T=1000, burn_in=0, thinning_rate=1):
    ...
    :param thinning_rate: Thinning of the chain to reduce autocorrelation between samples
    :return particles
    """
    # Determine dimension from initial state
